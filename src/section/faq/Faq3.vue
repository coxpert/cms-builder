<template>
  <div class="bz-section-container bz-sec--faq3-root" :class="{ [breakPoint]: true }" :data-section="section.sectionUUID">
        ...
        <bz-items v-model="data.items" :cols="setting.column" :shadow="false">
          <template slot-scope="{ index }">
            <div class="faq-question-wrapper">
              <bz-title v-if="setting.listElements.question" v-model="data.items[index].question" :text-color="theme.primaryColor" size="20px" />
            </div>
...
export default {
  mixins: [sectionMixin],
  components: { BzItems, BzText, BzTitle, BzContainer, BzBackground }
}
</script>
