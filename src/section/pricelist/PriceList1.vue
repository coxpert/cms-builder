<template>
  <div class="bz-section-container bz-sec--price-list-root" :class="{ [breakPoint]: true }">
      ...
      <bz-container>
        ...
        <bz-items v-model="data.items" :cols="setting.column" :shadow="setting.layouts.shadow">
          <template slot-scope="{ item, index }">
            <div class="bz-fl-pricing-card-top">
              <bz-alignment :alignment="setting.layouts.alignment">
                <bz-title v-model="data.items[index].title" v-if="setting.listElements.title" :background-color="theme.primaryColor" />

                <bz-text :text-color="'#ffffff'" v-model="data.items[index].price" v-if="setting.listElements.price" />

                <bz-text v-model="data.items[index].subtitle" v-if="setting.listElements.subtitle" :background-color="theme.primaryColor" />
              </bz-alignment>
            </div>
...
export default {
  components: { BzButton, BzItems, BzBackground, BzContainer, BzAlignment, BzTitle, BzText, BzSubtitle },
  mixins: [sectionMixin],
  name: 'PriceList1'
}
</script>
