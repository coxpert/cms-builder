<template>
  <div class="bz-section-container bz-sec--statistic-2-root" :class="{ [breakPoint]: true }" :data-section="section.sectionUUID">
        ...
        <bz-items v-model="data.items" :cols="setting.column" :shadow="setting.layouts.shadow">
          <template slot-scope="{ item, index }">
            <div class="bz-item-container">
              <bz-alignment :alignment="setting.layouts.alignment">
                <bz-text :bold="true" v-if="setting.listElements.value" v-model="data.items[index].value" :text-color="theme.primaryColor" :size="valueSize" :mb="5" />

                <bz-text v-if="setting.listElements.subtitle" v-model="data.items[index].subtitle" :mb="0" class="ml-2" />
              </bz-alignment>
            </div>
          </template>
        </bz-items>
