<template>
  <div class="bz-section-container bz-sec--usp-3-root" :class="{ [breakPoint]: true }" :data-section="section.sectionUUID">
    <bz-background :setting="background" :size="sectionSize" :bzAnimationClass="setting.animationClass">
      <bz-container>
        ...
        <bz-items v-model="data.items" :cols="setting.column" :shadow="setting.layouts.shadow" :spacing="setting.listElements.spacing">
          <template slot-scope="{ index }">
            <bz-alignment :alignment="setting.layouts.alignment" :style="{ paddingBottom: '20px' }">
              <div v-if="setting.listElements.icon" class="usp-item-container">
                <bz-icon v-model="data.items[index].icon" :text-color="'#ffffffff'" />
              </div>
              ...
              <bz-text v-if="setting.listElements.description" v-model="data.items[index].description" />
            </bz-alignment>
          </template>
        </bz-items>
      </bz-container>
...
export default {
  components: { BzIcon, BzDivider, BzItems, BzText, BzTitle, BzContainer, BzAlignment, BzBackground },
  mixins: [sectionMixin]
}
</script>
