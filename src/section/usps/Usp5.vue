<template>
  <div class="bz-section-container bz-sec--usp-5-root" :class="{ [breakPoint]: true }" :data-section="section.sectionUUID">
      ...
      <bz-container>
        ...
        <bz-items v-model="data.items" :cols="setting.column" :shadow="false" :spacing="false">
          <template slot-scope="{ index }">
            <bz-alignment :alignment="setting.layouts.alignment">
              <div class="bz-usp-item-container" v-if="setting.listElements.icon">
                <bz-icon v-model="data.items[index].icon" :text-color="theme.primaryColor" />
              </div>
