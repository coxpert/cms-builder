<template>
  <div class="bz-section-container bz-sect--section3-root" :class="{ [breakPoint]: true }" :data-section="section.sectionUUID">
    <bz-background :setting="background" :size="sectionSize" :bzAnimationClass="setting.animationClass">
      <bz-container>
        ...
        <bz-items v-model="data.items" :cols="setting.column" category-name="text" :shadow="setting.layouts.shadow">
          <template slot-scope="{ item }">
            <bz-card :background-color="'#ffffff'" :padding="20" :rounded="10">
              ...
              <div v-if="setting.listElements.buttons" class="d-flex justify-content-end">
                <bz-button-group v-model="item.buttons" />
              </div>
            </bz-card>
          </template>
        </bz-items>
