export default {
  init: function () {
      ...
      if (Array.isArray(fromObject)) {
        fromObject.forEach((item, index) => {
          if (typeof toObject[index] === "object") {
            toObject[index] = window._take(
              toObject[index],
              fromObject[index] || toObject[index]
            );
          } else {
    ...
    /**
     *  Set Theme colors with its variants as css variables
     * @param color
     * @param key
     */
    const setVariantsColors = (color, key) => {
      if (!color) {
        console.error(key + " color is not defined");
        return;
      }

      const textColor = window.getColor(color);
      const brightness = window.getBrightness(color);
      const lightColor = color.brighten(20);
      const darkColor = color.darken(20);
      const diffColor = brightness > 168 ? darkColor : lightColor;
      const hoverColor = diffColor;

      document.documentElement.style.setProperty(`--bz-${key}-color`, color);
    ...
    /*
     *------------------------------------------------------------------------------------------------------------------
     * Theme color setting as a css variables
     * Each color will have their variants, text-color, hover color, light-color, dark-color, diff color
     * -----------------------------------------------------------------------------------------------------------------
     *  @param theme
     */
    window._setThemeColors = (theme) => {
       ...
       *  Background colors
       */
      setVariantsColors(theme[theme.mode].backgroundColor, "background");

      /**
       *  Primary Colors
       */
      setVariantsColors(theme[theme.mode].primaryColor, "primary");

      /**
       *  Secondary Colors
       */
      setVariantsColors(theme[theme.mode].secondaryColor, "secondary");
      ...
      setVariantsColors(
        theme.isAdvanced
          ? theme[theme.mode].buttonColor
          : theme[theme.mode].primaryColor,
        "button"
      );
      ...
      setVariantsColors(
        theme.isAdvanced
          ? theme[theme.mode].socialIconColor
          : theme[theme.mode].primaryColor,
        "social-icon"
      );
      ...
      setVariantsColors(
        theme.isAdvanced
          ? theme[theme.mode].boxColor
          : theme[theme.mode].secondaryColor,
        "box"
      );
    };
    ...
    function preventDefault(e) {
      ...
      if (
        !window.isScrollable(window.scrollingTarget, direction) ||
        !(
          window.scrollingTarget === e.target ||
          window.scrollingTarget.contains(e.target)
        )
      ) {
        e.preventDefault();
      }
