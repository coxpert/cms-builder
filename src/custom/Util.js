  ...
  init: function (MediumEditor) {
    ...
    const Util = {
      ...
      extend: function extend(/* dest, source1, source2, ... */) {
        const args = [true].concat(Array.prototype.slice.call(arguments))
      ...
      splitEndNodeIfNeeded: function (currentNode, newNode, matchEndIndex, currentTextIndex) {
        let textIndexOfEndOfFarthestNode, endSplitPoint
      ...
      splitByBlockElements: function (element) {
        ...
        let toRet = []
        const blockElementQuery = MediumEditor.util.blockContainerElementNames.join(',')
        ...
        for (let i = 0; i < element.childNodes.length; i++) {
          ...
          if (child.nodeType === 3) {
            ...
            const blockElements = child.querySelectorAll(blockElementQuery)
      ...
      findAdjacentTextNodeWithContent: function findAdjacentTextNodeWithContent(rootNode, targetNode, ownerDocument) {
        ...
        let nextNode
        const nodeIterator = ownerDocument.createNodeIterator(rootNode, NodeFilter.SHOW_TEXT, null, false)
      ...
      insertHTMLCommand: function (doc, html) {
        ...
        let toReplace
        let res = false
        const ecArgs = ['insertHTML', false, html]
      ...
      execFormatBlock: function (doc, tagName) {
        ...
        const blockContainer = Util.getTopBlockContainer(MediumEditor.selection.getSelectionStart(doc))
        let childNodes
      ...
      addClassToAnchors: function (el, buttonClass) {
        ...
        if (el.nodeName.toLowerCase() === 'a') {
          ...
          const aChildren = el.getElementsByTagName('a')
          if (aChildren.length === 0) {
            const parentAnchor = Util.getClosestTag(el, 'a')
      ...
      splitOffDOMTree: function (rootNode, leafNode, splitLeft) {
        let splitOnNode = leafNode
        let createdNode = null
        const splitRight = !splitLeft

        // loop until we hit the root
        while (splitOnNode !== rootNode) {
          const currParent = splitOnNode.parentNode
          const newParent = currParent.cloneNode(false)
          let targetNode = splitRight ? splitOnNode : currParent.firstChild
          let appendLast
          ...
          while (targetNode) {
            const sibling = targetNode.nextSibling
      ...
      moveTextRangeIntoElement: function (startNode, endNode, newElement) {
        ...
        const rootNode = Util.findCommonRoot(startNode, endNode)
        if (!rootNode) {
          return false
        }

        if (endNode === startNode) {
          const temp = startNode.parentNode
          const sibling = startNode.nextSibling
        ...
        const afterLast = lastChild.nextSibling
      ...
      findCommonRoot: function (inNode1, inNode2) {
        let depth1 = Util.depthOfNode(inNode1)
        let depth2 = Util.depthOfNode(inNode2)
        let node1 = inNode1
        let node2 = inNode2
      ...
      getTopBlockContainer: function (element) {
        let topBlock = Util.isBlockContainer(element) ? element : false
      ...
      _getFirstTextNode: function (element) {
        ...
        for (let i = 0; i < element.childNodes.length; i++) {
          const textNode = Util._getFirstTextNode(element.childNodes[i])
          if (textNode !== null) {
            return textNode
          }
        }
