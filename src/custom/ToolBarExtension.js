module.exports = {
  init: function (MediumEditor) {
    ...
    const Toolbar = MediumEditor.Extension.extend({
      name: 'toolbar',
          ...
          if (extension && typeof extension.getButton === 'function') {
            ...
            if (Array.isArray(btn)) {
              for (const item of btn) {
                ...
                if (MediumEditor.util.isElement(item)) {
                  li.appendChild(item)
                } else {
                  li.innerHTML = item
                }
              ...
              if (extension.divide) {
                ...
                divider.className = 'medium-editor-divider'
      ...
      showToolbar: function () {
        if (this.buttons.includes('fontsize')) {
          const fontSizeExtension = this.base.extensions.find((ex) => ex.fontSizeBox)
          const fontSize = window.getComputedStyle(window.getSelection().anchorNode.parentElement, null).getPropertyValue('font-size')
          fontSizeExtension.fontSizeBox.innerHTML = parseInt(fontSize.replace('px', '')) + 'px'
        }

        if (this.buttons.includes('colorPicker')) {
          const colorPickerExtension = this.base.extensions.find((ex) => ex.name === 'colorPicker')
          if (typeof colorPickerExtension.updateForm === 'function') {
            colorPickerExtension.updateForm()
          }
        }
      ...
      hideToolbar: function () {
        if (this.isDisplayed()) {
          ...
          this.base.extensions.forEach((extension) => {
            if (extension.form) {
              extension.hideForm()
            }
          })
      ...
      multipleBlockElementsSelected: function () {
        const regexEmptyHTMLTags = /<[^\/>][^>]*><\/[^>]+>/gim // http://stackoverflow.com/questions/3129738/remove-empty-tags-using-regex
        const regexBlockElements = new RegExp('<(' + MediumEditor.util.blockContainerElementNames.join('|') + ')[^>]*>', 'g')
        const selectionHTML = MediumEditor.selection.getSelectionHtml(this.document).replace(regexEmptyHTMLTags, '') // Filter out empty blocks from selection
        const hasMultiParagraphs = selectionHTML.match(regexBlockElements) // Find how many block elements are within the html
      ...
      modifySelection: function () {
        ...
        if (this.standardizeSelectionStart && selectionRange.startContainer.nodeValue && selectionRange.startOffset === selectionRange.startContainer.nodeValue.length) {
          const adjacentNode = MediumEditor.util.findAdjacentTextNodeWithContent(
      ...
      checkState: function () {
        ...
        // hide toolbar
        const selectionElement = MediumEditor.selection.getSelectionElement(this.window)
      ...
      checkActiveButtons: function () {
        const manualStateChecks = []
        let queryState = null
        const selectionRange = MediumEditor.selection.getSelectionRange(this.document)
        let parentNode
        const updateExtensionState = function (extension) {
      ...
      setToolbarPosition: function () {
        const container = this.base.getFocusedElement()
        const selection = this.window.getSelection()
      ...
      positionStaticToolbar: function (container) {
        ...
        const OffsetToolbarVertical = 5

        if (this.sticky) {
          // If it's beyond the height of the editor, position it at the bottom of the editor
          if (scrollTop > containerTop + container.offsetHeight - toolbarHeight - this.stickyTopOffset) {
            toolbarElement.style.top = containerTop + container.offsetHeight - toolbarHeight - OffsetToolbarVertical + 'px'
            toolbarElement.classList.remove('medium-editor-sticky-toolbar')
            // Stick the toolbar to the top of the window
          } else if (scrollTop > containerTop - toolbarHeight - this.stickyTopOffset) {
            toolbarElement.classList.add('medium-editor-sticky-toolbar')
            toolbarElement.style.top = this.stickyTopOffset - OffsetToolbarVertical + 'px'
            // Normal static toolbar position
          } else {
            toolbarElement.classList.remove('medium-editor-sticky-toolbar')
            toolbarElement.style.top = containerTop - toolbarHeight - OffsetToolbarVertical + 'px'
          }
        } else {
          toolbarElement.style.top = containerTop - toolbarHeight - OffsetToolbarVertical + 'px'
        }
      ...
      positionToolbar: function (selection) {
        ...
        const defaultLeft = this.diffLeft - halfOffsetWidth
