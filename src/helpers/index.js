export default (function () {
  return {
    ...
    handlePMChanges(isPm, hours) {
      return isPm ? (hours === 0 ? 12 : hours) : hours === 12 ? 0 : hours;
    },
    ...
    createDisabledTimesArray({
      ...
      const reducer = (acc, val) => {
        ...
        const disabledTimeEntries = Array.isArray(disabledDatesAndTimes)
          ? disabledDatesAndTimes
          : [disabledDatesAndTimes];
        obj = forHours
          ? this.compareHours(disabledTimeEntries, obj, date)
          : this.compareMinutes(disabledTimeEntries, obj, date);
        if (obj.disabled) {
          acc.push(obj);
        }
        return acc;
    ...
    compareHours(disabledTimeEntries, obj, date) {
      disabledTimeEntries.forEach(({ to = null, from = null }) => {
        ...
        if (to && !from) {
          obj.disabled =
            toJSDate.getDate() === date.getDate()
              ? !(
                  toJSDate.getHours() <= date.getHours() &&
                  date.getMinutes() - toJSDate.getMinutes() <= 5
                )
              : !(toJSDate.getTime() <= date.getTime());
        } else if (!to && from) {
    ...
    filterMinutesWithSteps({ minutes, step, disabledMinutes }) {
      if (disabledMinutes && disabledMinutes.length > 0) {
        return minutes.map((m) =>
          m.point === disabledMinutes.find((d) => d.point === m.point)
            ? disabledMinutes.find((d) => d.point === m.point)
            : m.point % step === 0
            ? m
            : { point: m.point, disabled: true }
        );
      }
      return minutes.map((m) =>
    ...
    handleDegreeChange(isHourMode, isPm, timePoints, degree) {
      if (isHourMode) {
        ...
        if (isPm) {
          if (hours === 0) {
            return this.isHourDisabled(12, this.allHours)
