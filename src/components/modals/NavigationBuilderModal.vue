<template>
  <modal
    :classes="['navigation-builder-modal']"
      ...
      <Tree v-model="data" :fold-all-after-mounted="true">
        <template v-slot="{ node, index, path, tree }">
          <div class="children-item">
            <div
              v-if="showExpandIcon(tree, path)"
              class="expand-icon"
              @click.prevent="tree.toggleFold(node, path)"
            >
            ...
            <div class="children-image">
              <img
                v-if="tree.getNodeByPath(path).image"
                :src="tree.getNodeByPath(path).image.src"
              />
            </div>
            <div class="children-icon">
              <i
                v-if="tree.getNodeByPath(path).icon"
                :class="tree.getNodeByPath(path).icon"
            ...
            <div class="action-buttons">
              <div
                ...
                @click.prevent="addNewSubmenu(node, path, tree)"
              >
              ...
              <div
                class="action-button-item"
                @click.prevent="openIconSelector(path, tree)"
              >
                <dvr-icon :size="18" />
              </div>
              <div
                class="action-button-item"
                @click.prevent="openAttachLinkModal(path, tree)"
              >
                <link-icon :size="18" />
              </div>
              <div
                class="action-button-item"
                @click.prevent="openSelectImage(path, tree)"
              >
                <image-icon :size="18" />
              </div>
              <div
                class="action-button-item delete"
                @click.prevent="deleteNavItem(path, tree)"
              >
                <delete-icon :size="18" />
              </div>
            </div>
  ...
  methods: {
    ...
    addNewSubmenu(node, path, tree) {
      ...
      if (nodeData.children) {
        ...
        nodeData.children = [
          { name: "new item", $folded: false, children: [] },
        ];
      }
      this.refresh();
