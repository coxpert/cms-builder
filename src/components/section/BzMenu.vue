      ...
      <div class="bz-el-menu">
        <template v-if="edit">
          <template v-for="(page, index) of allPages">
            <div
              v-if="showMenuItem(page) && index <= showMenuIndex"
              ref="menuItems"
              :key="index"
            ...
            <template v-if="edit">
              <template v-for="(page, index) of allPages">
                <div
                  ...
                  :key="index"
                  @click.stop.prevent="
                    setActivePage({ index, type: page.type })
                  "
                >
                  <slot
                    :page-name="page.name"
                    :active="isActiveMenu(page.url)"
                  />
                </div>
              </template>
            </template>
      ...
      new ResizeObserver(() => {
        ...
        for (let i = 0; i < self.$refs.menuItems.length; i++) {
          ...
          if (
            menuItemsWidth >
            Math.min(
              this.maxWidth || self.$el.clientWidth - 170,
              self.$el.clientWidth - 170
            )
          ) {
