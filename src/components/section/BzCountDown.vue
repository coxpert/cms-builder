<template>
  <div
    class="bz-el--count-down-root"
    :style="{ fontFamily: theme.paragraphFont }"
  >
    <slot />
...
import BzDatetimePicker from "./BzDateTimePicker";
...
export default {
  ...
  computed: {
    countTime: {
      get() {
        return this.data || this.$moment().format("YYYY-MM-DD H:m:s");
      },
      set(val) {
        this.data = val;
      },
