<template>
  <div
    class="bz-el--form-group-root"
    :class="{ stretch, 'bz-d-lg-flex': inline }"
  >
    <div :class="{ 'bz-w-lg-30': inline }">
      <label
        v-if="label"
        class="bz-form--label"
        :style="{ color: color, fontFamily: theme.paragraphFont }"
      >
        {{ label }}
      </label>
    </div>
    ...
    <div :class="{ 'bz-w-lg-70': inline }">
      <input
        v-if="type === 'text'"
        v-model="data"
        class="bz-form-item bz-form--input"
        :style="{
          borderColor: color,
          color: color,
          fontFamily: theme.paragraphFont,
        }"
      />
      <textarea
        v-if="type === 'textarea'"
