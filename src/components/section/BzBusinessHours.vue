<template>
  <div
    class="bz-business-hours-root"
    :style="{ fontFamily: theme.paragraphFont, color: color }"
    :class="{ edit }"
  >
    <div :class="'business-hour-wrapper-' + design">
      <div v-for="hour of businessHours" :key="hour.day" class="open-hour-item">
        <div
          class="day"
          :style="{ fontFamily: theme.titleFont, color: dayColor }"
        >
          {{ hour.day }}
        </div>
        ...
        <div class="label">
          {{ hour.openHour.label }}
          <small>{{ hour.openHour.detail ? hour.openHour.type : "" }}</small>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    businessHours() {
      const businessHours = [];
      let selectedBusiness = this.templateSetting.businesses.find(
      ...
      if (selectedBusiness) {
        for (const key in selectedBusiness.businessHours) {
          if (businessHours.length > 0) {
            ...
            if (
              lastBusinessHour.openHour.type ===
                selectedBusiness.businessHours[key].type &&
              lastBusinessHour.openHour.label ===
                selectedBusiness.businessHours[key].label
            ) {
              lastBusinessHour.day =
                lastBusinessHour.day.split("-")[0] + "-" + key;
              continue;
            }
          }
          ...
          businessHours.push({
            ...
            openHour: {
              label,
              type: selectedBusiness.businessHours[key]?.type,
              detail: selectedBusiness.businessHours[key]?.dayTime,
            },
          });
        }
      }
