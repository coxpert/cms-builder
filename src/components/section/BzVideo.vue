  ...
  <div class="bz-el-video-root" :class="{ [breakPoint]: true }">
    <div v-if="edit" class="tool-bar" :class="{ editing: editing }">
      <div class="input-group mb-2">
        <input type="text" v-model="youtubeURL" class="form-control" placeholder="Youtube URL here" />
        <div class="input-group-postpend">
          <a href="javascript:void(0)" type="button" class="btn btn-primary text-white rounded-0 text-decoration-none" v-tooltip="'Save video URL'" @click.prevent="changeLink()">
            Change Link
          </a>
        </div>
      </div>
    </div>
    <bz-setting :on-click="openVideoModal">
