<template>
  <div class="bz-el--date-root" :class="{ edit }">
    ...
    <div class="bz-date-editor" v-if="edit">
...
export default {
  ...
  computed: {
    date: {
      get() {
        if (this.data) {
          return this.$moment(new Date(this.data)).format(this.format);
        }
        return this.$moment().format(this.format);
      },
      set(val) {
        this.data = this.$moment(new Date(val)).format(this.format);
      },
    },
    _year: {
      get() {
    ...
    _month: {
      get() {
        if (this.date) {
          return this.$moment(new Date(this.date)).format("MMMM");
        }
