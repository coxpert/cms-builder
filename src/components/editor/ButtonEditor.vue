<template>
  <div id="bz-button-editor" v-click-outside="handleOutsideClick" :style="{ left: left + 'px', top: top + 'px' }" class="bz-button-editor-root">
    <template v-if="link">
    ...
    <template v-if="multiple">
      ...
      <div v-tooltip="'Move Button'" class="item-wrapper" :class="{ disabled: start }" @click="moveToLeft">
        <div class="item icon">
          <arrow-back-icon :size="16" :fill-color="start ? '#8080807f' : '#555555'" />
        </div>
      </div>

      <div v-tooltip="'Move Button'" class="item-wrapper" :class="{ disabled: last }" @click="moveToRight">
        <div class="item icon">
    ...
    <div v-if="openColorForm" v-click-outside="closeColorForm" class="color-form-root">
      <template v-for="color of editableBackgroundColors">
        <div :key="color" class="color-item-wrapper" @click.prevent="handleChangeColor(color)">
      ...
      <bz-divider :background-color="'#fffff'" :vertical="true" :height="20" />
      <div class="color-item-wrapper" @click.prevent="handleChangeColor(customColor)">
...
export default {
  name: 'ButtonEditor',
  components: { BzDeleteIcon, BzColorizeIcon, LinkIcon, Sketch, PaletteIcon, ArrowForwardIcon, ArrowBackIcon, AddIcon, DvrIcon, BzDivider },
  mixins: [editorMixin],
  props: {
    multiple: {
      type: Boolean,
  ...
  computed: {
    ...
    colorItemOpacityBackground() {
      const originalColor = window.tinycolor(this.data.backgroundColor || this.theme.primaryColor)
