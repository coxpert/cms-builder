<template>
  <div class="bz-business-hours-root" :class="{ edit }">
    ...
    <div
      v-for="(hour, day) of businessHours"
      :key="day"
      class="w-100 open-hour-item"
    >
        ...
        <div class="hour-item">
          ...
          <div class="d-flex align-items-center justify-content-center">
            <template v-if="hour.dayTime">
              <div class="day-time">
                <div class="start-time" @click="editDayStartTime(day)">
                  {{ hour.dayTime.start }}
                </div>
                <span>-</span>
                <div class="end-time" @click="editDayEndTime(day)">
                  {{ hour.dayTime.end }}
                </div>
              </div>
            </template>
            ...
            <template v-if="hour.nightTime">
              <div class="night-time">
                <div class="start-time" @click="editNightStartTime(day)">
                  {{ hour.nightTime.start }}
                </div>
                <span>-</span>
                <div class="end-time" @click="editNightEndTime(day)">
                  {{ hour.nightTime.start }}
                </div>
              </div>
            </template>
...
export default {
    ...
    appointmentCategory: {
      ...
      handler(value) {
        if (value !== "" && typeof value === "number" && value !== 0) {
          const tempAppointmentCategory = this.appointmentCategories.find(
            (o) => o.value === value
          );
          ...
          tempAppointmentCategory.weekdays.forEach(function (obj) {
            ...
            if (typeof hours === "undefined") {
              ...
              tempDayTime = {
                start: tempTime24To12(hours.start),
                end: tempTime24To12(hours.end),
              };
            }

            if (obj.weekday === "sun") {
              tempBusinessHours.sunday = {
                type: "appointment only",
                dayTime: tempDayTime,
              };
            } else if (obj.weekday === "mon") {
              tempBusinessHours.monday = {
                type: "appointment only",
                dayTime: tempDayTime,
              };
            } else if (obj.weekday === "tue") {
              tempBusinessHours.tuesday = {
                type: "appointment only",
                dayTime: tempDayTime,
              };
            } else if (obj.weekday === "wed") {
              tempBusinessHours.wednesday = {
                type: "appointment only",
                dayTime: tempDayTime,
              };
            } else if (obj.weekday === "thu") {
              tempBusinessHours.thursday = {
                type: "appointment only",
                dayTime: tempDayTime,
              };
            } else if (obj.weekday === "fri") {
              tempBusinessHours.friday = {
                type: "appointment only",
                dayTime: tempDayTime,
              };
            } else if (obj.weekday === "sat") {
              tempBusinessHours.saturday = {
                type: "appointment only",
                dayTime: tempDayTime,
              };
            }
  ...
  mounted() {
    const selectedBusiness = this.templateSetting.businesses.find(
