  ...
  <div class="container">
      ...
      <div class="col-md-6 text-center">
        <div style="width: 500px; height: 500px" ref="svg">
          ...
          >
            ...
            <rect
              v-if="attrs.type === 'linear'"
              x="0"
              y="0"
              width="100%"
              height="100%"
              :fill="'url(#LinearGradient' + lid + ')'"
            />
            <rect
              v-if="attrs.type === 'radial'"
              x="0"
              y="0"
              width="100%"
              height="100%"
              :fill="'url(#RadialGradient' + rid + ')'"
            />
          </svg>
        </div>
      </div>
        ...
        <div v-if="attrs.points">
          <table class="table table-centered table-bordered">
            ...
            <tr v-for="(attr, i) in attrs.points" :key="i">
              <td>{{ i + 1 }}</td>
              <td>
                <input type="number" v-model="attr.left" min="0" max="100" />
              </td>
              <td>
                <input
                  type="text"
                  v-bind:value="getHexRow(attr)"
                  @change="setHexRow(attr)"
                />
              </td>
              <td>
                <input
                  type="number"
                  v-bind:value="attr.red"
                  @change="setRGBRow(attr, 'red')"
                  min="0"
                  max="255"
                />
              </td>
              <td>
                <input
                  type="number"
                  v-bind:value="attr.green"
                  @change="setRGBRow(attr, 'green')"
                  min="0"
                  max="255"
                />
              </td>
              <td>
                <input
                  type="number"
                  v-bind:value="attr.blue"
                  @change="setRGBRow(attr, 'blue')"
                  min="0"
                  max="255"
                />
              </td>
              <td>
                <input
                  type="text"
                  v-bind:value="getAlphaRow(attr.alpha)"
                  @change="setAlphaRow(i)"
                />
              </td>
            </tr>
          </table>
    ...
    <div class="row">
      ...
      <div class="col-md-6">
        <a
          href="#"
          class="ml-auto btn m-btn--square btn-outline-primary mb-2"
          @click.prevent="gotoBack"
          >Back</a
        >
        <a
          href="#"
          class="ml-auto btn m-btn--square btn-outline-success mb-2"
          @click.prevent="submit"
          v-bind:disabled="formBusy"
          >Submit</a
        >
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    getColor(red, green, blue) {
      ...
      return (
        prefix +
        this.getHexNumber(red) +
        this.getHexNumber(green) +
        this.getHexNumber(blue)
      );
    },
    ...
    getHexRow(attr) {
      return (
        this.getHexNumber(attr.red) +
        this.getHexNumber(attr.green) +
        this.getHexNumber(attr.blue)
      );
    },
    getAlphaRow(alpha) {
      return alpha * 100;
